...
(function () {
                        ...
                        for (var e = d.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || d.name == "body", h, f = g.length; f > 0;) {
                            ...
                            if (e && (p.type == CKEDITOR.NODE_TEXT || p.type == CKEDITOR.NODE_ELEMENT && A.$inline[p.name])) {
                                ...
                                d.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT && (p.type ==
                                    CKEDITOR.NODE_ELEMENT && !A[d.name][p.name]) && c.push({
                                    check: "el-up",
                                    el: p
                                })
                            }
            ...
            for (e in g)
                if (e == "class") {
                    ...
                    for (p = a.classes.join("|"); h =
                        c.pop();)
                        if (p.indexOf(h) == -1) return false
                } else if (a.attributes[e] != g[e]) return false;
            for (f in d)
                if (a.styles[f] != d[f]) return false;
            return true
        }
            ...
            applyTo: function (a, b, c) {
                ...
                a.forEach(function (a) {
                    if (a.type == CKEDITOR.NODE_ELEMENT) {
                        ...
                        var c;
                        a: {
                            var g = decodeURIComponent(a.value.replace(/^\{cke_protected\}/, ""));c = [];
                            var f, n, o;
                            if (l)
                            ...
                            g = CKEDITOR.htmlParser.fragment.fromHtml(g);g.children.length == 1 && (f = g.children[0]).type == CKEDITOR.NODE_ELEMENT && h(f, e, p, c, b);c = !c.length
                        }
                        c || d.push(a)
                    }
                }, null, true);
                ...
                h = function (a) {
                    b.call(this, a, function (a) {
                        ...
                        for (j in a) {
                            ...
                            if (!c[j]) {
                                if (q.icons)
                                    for (var l = q.icons.split(","), g = l.length; g--;) CKEDITOR.skin.addIcon(l[g], q.path + "icons/" + (CKEDITOR.env.hidpi && q.hidpi ? "hidpi/" : "") + l[g] + ".png");
                                c[j] = 1
                            }
                            if (r) {
                                r.split && (r = r.split(","));
            ...
            CKEDITOR.plugins.load(c.split(","), function (c) {
                ...
                for (var l in c) {
                    ...
                    if (n && !a.lang[l]) {
                        ...
                        if (!q.langEntries ||
                            !q.langEntries[o]) f.push(CKEDITOR.getUrl(q.path + "lang/" + o + ".js"));
                        else {
    ...
    (function () {
                ...
                return function (d) {
                    if (d.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT) {
                        ...
                        if (l = CKEDITOR.env.opera && !b || (typeof c == "function" ? c(d) !== false : c))
                            if (!q && CKEDITOR.env.ie && d.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT) l = false;
        ...
        var F = /(?:&nbsp;|\xa0)$/,
            ...
            z = CKEDITOR.tools.extend({}, s.$blockLimit, s.$block),
            t = {
                elements: {},
                attributeNames: [
                    [/^on/, "data-cke-pa-on"]
                ...
                insertElement: function (b) {
                    ...
                    for (var c = this.editor, d = c.config.enterMode,
                        ...
                        if (!o.checkReadOnly()) {
                            ...
                            if (j)
                                for (;
                                    (x = o.getCommonAncestor(0, 1)) && (z = CKEDITOR.dtd[x.getName()]) && (!z || !z[i]);)
                                    if (x.getName() in CKEDITOR.dtd.span) o.splitElement(x);
                    ...
                    if (k) {
                        ...
                        if (j)
                            if ((b = k.getNext(a)) && b.type == CKEDITOR.NODE_ELEMENT && b.is(CKEDITOR.dtd.$block)) b.getDtd()["#"] ? o.moveToElementEditStart(b) : o.moveToElementEditEnd(k);
        ...
        var m = function () {
            ...
            return function (o, k, z) {
                ...
                if (!C.checkReadOnly()) {
                    ...
                    for (;
                        (D = b(t.startContainer) && t.startContainer.getChild(t.startOffset - 1)) && b(D) && D.isBlockBoundary() && I.contains(D);) t.moveToPosition(D, CKEDITOR.POSITION_BEFORE_END);
                        ...
                        if (!t || i[t] && !u && (!o || h) && (m.getPosition(l) | CKEDITOR.POSITION_PRECEDING | CKEDITOR.POSITION_IDENTICAL | CKEDITOR.POSITION_IS_CONTAINED) == CKEDITOR.POSITION_PRECEDING + CKEDITOR.POSITION_IDENTICAL + CKEDITOR.POSITION_IS_CONTAINED && (!e.childRule || e.childRule(m))) {
                            ...
                            if (v && ((v.getDtd() || CKEDITOR.dtd.span)[d] || f) && (!e.parentRule || e.parentRule(v))) {
                                ...
                                if (s == CKEDITOR.NODE_TEXT || o || s == CKEDITOR.NODE_ELEMENT && !m.getChildCount()) {
                                    for (var s = m, x;
                                        (g = !s.getNext(C)) && (x = s.getParent(), i[x.getName()]) && (x.getPosition(j) | CKEDITOR.POSITION_FOLLOWING | CKEDITOR.POSITION_IDENTICAL | CKEDITOR.POSITION_IS_CONTAINED) == CKEDITOR.POSITION_FOLLOWING + CKEDITOR.POSITION_IDENTICAL + CKEDITOR.POSITION_IS_CONTAINED && (!e.childRule ||
                                            e.childRule(x));) s = x;
                                    q.setEndAfter(s)
                                }
                            } else g = true
                        ...
                        for (var g = p(this, c), o = g.hasAttributes(), u = q.getCommonAncestor(), s = {}, t = {}, v = {}, z = {}, y, A, F; g && u;) {
                            if (u.getName() == d) {
                                for (y in e.attributes)
                                    if (!z[y] && (F = u.getAttribute(A))) g.getAttribute(y) == F ? t[y] = 1 : z[y] = 1;
                                for (A in e.styles)
                                    if (!v[A] && (F = u.getStyle(A))) g.getStyle(A) == F ? s[A] = 1 : v[A] = 1
                            }
                            u = u.getParent()
                        }
        ...
        function q(a) {
            ...
            if (l) {
                var b = CKEDITOR.document.getBody().append(CKEDITOR.dom.element.createFromHtml(u.output({
                    ...
                    p = CKEDITOR.tools.eventsBuffer(500,
                        o),
